...
| Current authoritative activity    | Version PR #38's pending correction, publish the resulting coordinated alpha after the npm-token rotation, advance App [PR #114](https://github.com/kumwe/app/pull/114) against that exact family, complete `authoring-web`, and assemble independently reproduced evidence                                   |
| Repository verification baseline  | `dadb69a904ef774e974fc614832c5b6b01d1f6f1`: PR #38 is merged; its renderer/rich-text runtime, release records, and corpus corrections preserve the coordinated `0.1.0-alpha.9` family, and the `main` Quality and Accessibility jobs pass                                                                     |
| Next package coordinate           | The alpha release plan is `version`: consume PR #38's pending patch Changeset through the generated whole-family version PR, then publish and verify the resulting numeric alpha coordinate. Do not bypass the new correction by recovering the superseded `alpha.9` tree                                     |
...
| `M1-05` | evidence-review | Secret scan, audit, SBOM, release train, changeset gate, coordinated `alpha.9` version commit                                                                               | Reviewer reproduction; version PR #38's pending correction, then complete the first eight-package registry publish |
...
1. Let the alpha train consume PR #38's pending patch Changeset in one generated whole-family version PR.
   After that PR merges, the next exact current-`main` run must publish the retained approved tarballs under
   ...
   superseded `alpha.9` tree, a workspace tree, or a subset of packages in its place.
