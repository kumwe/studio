...
**Publication inventory checked:** 2026-09-12

Implementation and gate evidence below retain their recorded baselines. This publication refresh does not
change product-requirement states or accept new qualification evidence.
...
| Item                              | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Programme phase                   | The configuration-driven browser deployment, isolated local defaults, exact hosted HTTP transport, contextual start/save coordinator, prebuilt browser archive, and reference PHP security boundary are repository-verified at `38a96472ff4a5e1aa1fb92ed5451dc0fd112cf48`. Product completion still requires the remaining composed authoring UI, extension lifecycle journey, preview staging protocol, qualified Producer realization and Producer-backed Kumwe App adapter, complete `authoring-web` journey, and external qualification              |
...
| Published Studio packages         | All eight npm `beta` coordinates are `0.1.0-beta.4`. The [coordinated GitHub prerelease](https://github.com/kumwe/studio/releases/tag/studio-v0.1.0-beta.4), published 2026-09-07, resolves to `ec8a448f674166562178f36e3ee65a68563c7087` and includes the browser archive and detached checksum. This inventory is an observed publication fact, not independent gate acceptance.                                                                                                                                                                       |
| Declared conformance profiles     | The current [release record](../../studio-release.json) carries no profile claims. The complete fixed Version 2 set requires all product requirements and profile assertion sets to be repository-executable before a new RC is prepared.                                                                                                                                                                                                                                                                                                                |
...
| Release channel                   | Governed beta development. The current coordinated source record is `0.1.0-beta.4`; only generated versioning changes manifests, lockfile, changelogs, prerelease state or release-record copies. The `rc` and stable channels remain blocked.                                                                                                                                                                                                                                                                                                           |
...
| Current authoritative activity    | Finish the Studio-side composed authoring UI and protocol gaps against the landed configuration-first boundary: all three save outcomes, presentation continuity, declarative extension block/field-adapter lifecycle, preview-draft staging, external-media/status UX, and the complete `authoring-web` journey. Then deliberately re-pin and prove that exact release through Kumwe Producer before binding Producer to Kumwe App PHP application services, with no production Node.js/npm, and obtain external/manual evidence and independent review |
| Historical verification baseline  | `38a96472ff4a5e1aa1fb92ed5451dc0fd112cf48` passes `npm run verify` for the configuration-driven browser/PHP increment: 1,290 Vitest tests, 179 script tests, 18 PHP reference tests, and 19 Playwright journeys. It is the published coordinated `0.1.0-beta.2` development family with no profile, host-support, gate, RC, or production-support claim. Gate A remains **Not assessed** and Gate B remains **Blocked**                                                                                                                                  |
| Candidate package coordinate      | None. The published `0.1.0-beta.4` family is a development release. The next eligible candidate for this semantic base is `0.1.0-rc.2`, subject to the complete implementation and profile guards in CONTRIBUTING.md.                                                                                                                                                                                                                                                                                                                                    |
...
| `STUDIO-PROD-010` | `active`      | Exact operation-map/single-endpoint transport plus PHP session/CSRF/token enforcement is verified; qualified Producer wire realization plus real Kumwe App application-service replay and preview staging remain |
...
| `M1-05` | active          | Secret scan, audit, SBOM, retry-safe staged publication, immutable quarantined `0.1.0-rc.1` history, fail-closed RC/stable controls, and verified npm publication of coordinated `0.1.0-beta.2`                                                                                                                                        | Verify the selected coordinated release through the governed workflow; complete all 15 product requirements and all nine executable profile assertion sets; then prepare `0.1.0-rc.2` and qualify it through protected evidence and review |
...
| `M3-05` | active          | Six canonical contribution kinds plus exact hosted catalog locks, target admission, and owner-aware generation are schema-backed and runtime-checked                                                                                                                                                                                   | Complete extension target/block/field-adapter disable-upgrade-migration journey plus qualified Producer and Producer-backed Kumwe App PHP-authoritative adapter and independent host replay (`STUDIO-PROD-008`–`010`)                      |
...
| ----------------------------------------------------------------------------------------------------- | ------------------------------------------ | ---------------------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `M1-01`, `M3-05`, `M4-01`, `M4-03`, `M4-04`, `M5-01`, `M5-03` / configuration-first browser authoring | `38a96472ff4a5e1aa1fb92ed5451dc0fd112cf48` | none — not yet created | active          | Finish the composed UI, extension lifecycle, preview staging, complete `authoring-web`, qualified Producer and Producer-backed Kumwe App PHP adapter, independent deployed-host replay, manual evidence, and reviewer reproduction | unassigned |
| `M2-06` / `gate-a/09-typescript-corpus`                                                               | `38a96472ff4a5e1aa1fb92ed5451dc0fd112cf48` | none — not yet created | evidence-review | Reproduce the cross-locale generated-source drift check, 241 exact-root assignments, two named `TS2321` depth boundaries, and 243-document schema-validated JSON round-trip in an immutable Gate A bundle                          | unassigned |
...
Kumwe Producer and Kumwe App remain real external dependencies, but this repository ships no Producer- or
Kumwe App-specific code. Producer must deliberately vendor and digest-verify one exact published Studio family,
release/schema/corpus closure, conformance vectors, and prebuilt assets, then replay the required corpora in
PHP. Kumwe App consumes that exact Studio release through the resulting exact Producer release and binds its
real authority, storage, PHP HTTP, and Twig/KIS delivery services behind Producer. A workspace snapshot,
provisional host-private target shape, partial Producer source pin, or TypeScript testbed cannot satisfy that
...
3. Review the coordinated beta prerelease change and, only through an exact-SHA governed recovery run, reconcile
   the missing `studio-v0.1.0-beta.2` GitHub prerelease/tag and approved browser archive/checksum. Do not create
   or move that tag manually. Node.js and npm remain contributor/build/release inputs only; the archive and PHP
   integration must never require a production JavaScript server or client-side package installation.
4. Deliberately re-pin that exact beta record, schemas, corpus/vectors, authoring archive, and enhancement
   runtime in Producer; verify every digest and replay the canonical, renderer-web, and rich-text corpora in
   PHP. Publish that Producer increment, atomically pin it in Kumwe App, implement the published operation map
   through real App PHP application services and PHP HTTP endpoints, emit one bounded deployment per content
   mount, and replay host, preview, media, target, contribution, binding, property-schema, renderer, conflict,
   authentication, and authorization behavior through the real Producer/App PHP/Twig stack.
...
6. Activate the reviewed external reviewer authority, generate candidate-bound bundles, obtain independent
   ...
   remain blocked until the full production matrix and integrated journey are qualified. There is no alternate
   beta publisher.
...
The historical quarantined `0.1.0-rc.1` label is preserved but abandoned because its product maturity was
overstated; the next eligible candidate on this base is `0.1.0-rc.2`.
