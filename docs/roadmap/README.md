...
7. Public rendering never depends on the Studio authoring runtime. The reference Kumwe App profile renders
   through Producer/PHP and App Twig/KIS chrome, preserves the semantic no-JavaScript result, and includes
   Studio's one prebuilt deferred enhancement runtime only when the renderer's `enhancements` signal requires
   one of its eight public behavior families.
...
12. Kumwe App keeps every authoritative effect in PHP application services and PHP HTTP endpoints behind the
    exact pinned Producer realization. Compiled Studio browser assets require zero production Node.js/npm
    runtime.
...
| `M3-05 Producer-backed Kumwe App integration seams` | Gate A                      | Producer deliberately pins and proves one Studio release, then Kumwe App binds contextual target launch and authoritative PHP host ports/API surfaces additively while current editors remain transitional fallback. The integration preserves contribution ownership, immutable generations, workflow, revisions, policy, audit, Producer rendering in App Twig/KIS chrome, recovery, strict CSP, and zero production Node.js/npm. |
...
| `M5-06 Producer-backed Kumwe App vertical slice` | `M3-05`, `M5-03`                  | Kumwe App completes the same journey through the exact Producer/Studio pin, extension-declared targets, PHP-authoritative App services, Producer public rendering in App Twig/KIS chrome, typed field/media bindings, real persistence/workflow/audit, compiled browser assets, and zero production Node.js/npm. |
...
15. **`gate-b/15-kumwe-app-vertical`** — Kumwe App completes `STUDIO-PROD-015` through the exact
    Producer/Studio pin, PHP-authoritative App content, type, extension, media, workflow, revision, audit, API,
    and Twig/KIS integration with zero production Node.js/npm.
