...
The reference preview sends the current draft to an authenticated host preview port and renders it with the same trusted block/theme renderer used for delivery, such as Kumwe's Twig components. A sandboxed iframe returns opaque node markers and measurements to Studio.
