...
| Block-local controls    | Editor.js `2.31.6` behind Studio's private factory, first-party structured tools, canonical rich-text JSON, Markdown conversion, safe-HTML import, and a strict-CSP sink-free surface | The Lit shell is a separate alpha surface with read-only model projection; these controls are not a coordinated accessible Entry-value editor (`STUDIO-PROD-003`, `013`)                   |
