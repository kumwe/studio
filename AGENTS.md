...
Release work follows the same single lifecycle: automatic coordinated beta increments, the manual governed
RC/stable workflow, and evidence bundles used only as supporting qualification records. The exact inputs,
correction path, next-beta behavior, and recovery rules are in `CONTRIBUTING.md`; do not invent another
